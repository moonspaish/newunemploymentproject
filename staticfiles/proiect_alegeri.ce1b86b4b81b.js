...
function createBarPlot(votesData) {
    ...
    // Append x-axis but remove the labels, ticks, and axis line
    svg.append("g")
    ...
    svg.selectAll(".tick line").remove(); // Remove the ticks

    // Add text labels inside or outside the bars based on top 4 values
    svg.selectAll("text.label")
        ...
        .attr("x", d => {
            ...
            // Determine if the value is one of the top 4
            return top4Values.includes(d.votes) ? xPos - 5 : xPos + 5;
        })
        .attr("y", d => y(d.candidate) + y.bandwidth() / 2 + 5)  // Center vertically within the bar
        .attr("fill", d => top4Values.includes(d.votes) ? "white" : "black")  // White for inside, black for outside
        .attr("text-anchor", d => top4Values.includes(d.votes) ? "end" : "start")  // Align inside or outside
        .text(d => d.votes);
}
